...
class FeedbackReceivedScreen extends React.Component {
  constructor(props) {
    super(props);
    this.state = {};
  }
      ...
      <MCView bordered br={10} mb={10}>
        <MCCard shadow br={1} style={{width: '100%'}}>
          <MCView row align="center">
            ...
            <H3 ml={10}>{feedback.receiver.name}</H3>
          </MCView>
        </MCCard>
  ...
  render() {
    const {receivedFeedbacks} = this.props;
    return (
...
const mapStateToProps = state => ({
  ...
  receivedFeedbacks: selector.feedbacks.getMyFeedbacks(state).received,
});

const mapDispatchToProps = {};

export default withTranslation()(
  connect(mapStateToProps, mapDispatchToProps)(FeedbackReceivedScreen),
);
